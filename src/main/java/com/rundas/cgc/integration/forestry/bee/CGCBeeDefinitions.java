    ...
    public static void defineBees(IApicultureRegistration apicultureRegistration) {
        for (Material material : MaterialRegistryManager.getInstance().getRegisteredMaterials()) {
            if (material.hasProperty(CGCPropertyKeys.BEE)) {
                ...
                IBeeSpeciesBuilder builder = apicultureRegistration.registerSpecies(CreatingGregCore.id("bee_" + material.getUnlocalizedName().split("\\.")[2]), property.genus(), material.getName().toUpperCase(Locale.ENGLISH), property.isShiny(), TextColor.fromRgb(property.primaryColor()));
